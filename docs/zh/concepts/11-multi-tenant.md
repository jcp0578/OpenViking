...
- 默认推荐场景下，不需要额外配置 `accountId` / `userId`
...
- `accountId` / `userId`
  - 仅在插件使用 root key，或服务端运行在 `trusted` 模式时需要
  - 普通 user key 场景下通常不需要配置
- `agentScopeMode`
  - 默认不需要配置
  - 仅当服务端把 `memory.agent_scope_mode` 改成非默认值时，插件才需要同步配置为相同语义
  - 当前服务端仍要求部分 `agent` 逻辑 URI 解析到显式 space，因此这个字段目前是兼容项，不是默认项
